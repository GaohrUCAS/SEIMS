...
IMP_SWAT::IMP_SWAT(void) : m_cnv(NODATA_VALUE), m_nCells(-1), m_cellWidth(NODATA_VALUE), m_cellArea(NODATA_VALUE), 
	...
	m_slope(NULL), m_ks(NULL), m_sol_fc(NULL), m_sol_sumfc(NULL), m_soilThick(NULL), m_sol_por(NULL), 
...
/// initialize temporary variables
	...
	if (m_potVol[id] > UTIL_ZERO)
	{
		...
		drtot = (m_potClay[id] + m_potSilt[id] + m_potSand[id] + m_potSag[id] + m_potLag[id]) / 
			(m_potClayIn + m_potSiltIn + m_potSandIn + m_potSagIn + m_potLagIn);
		m_potSed[id] *= drtot;
		...
		//yy = m_ks[id][0];
		/// calculate seepage into soil // these code from SWAT seems problemetic. By LJ
		...
		{
			float dep2cap = m_sol_fc[id][ly] - m_soilStorage[id][ly];
			if (dep2cap > 0.f)
			{
	...
	if (id == 1085)  /// dianbu 8144, dianbu2 1085
		cout<<m_surfaceRunoff[id]<<", potVol: "<<m_potVol[id]<<endl;
}
