...
# sol_stap(:,:) |kg P/ha: amount of phosphorus in the soil layer stored in
# the stable mineral phosphorus pool
...
def SoilChemProperties(nlyrs, depth, om, clay, rock, bd):
    ...
    # phosphorus stored in the fresh organic (residue) pool
    sol_fop = numpy.ones((nlyrs))
    # nitrogen stored in the fresh organic (residue) pool
    sol_fon = numpy.ones((nlyrs))
    ...
    # Organic matter in the soil layer classified as residue, currently
    # assigned 0. TODO
    ...
    for lyr in range(nlyrs):
        sol_mass[lyr] = (sol_thick[lyr] / 1000.) * 10000. * 1000. * bd[lyr] * (
            1 - rock[lyr] / 100.)  # soil mass of current layer (kg)
        # Carbon mass of current layer (kg)
        sol_cmass[lyr] = sol_mass[lyr] * (cbn[lyr] / 100.)
        if lyr == 0:
    ...
    for j in range(nlyrs):
        ...
        if (sol_no3[j] <= 0.):
            sol_no3[j] = 0.
            # No need to divided by 1000., because the unit of depth is mm!
            # zdst = math.exp(-depth[j]) in SEIMS
        ...
        if (conv_wt != 0):  # From White et al 2009
            ...
            solp = sol_solp[j] / conv_wt * 1000000.
            # estimate Total Mineral P in this soil based on data from sharpley
            # 2004
    ...
    return (list(sol_fop), list(sol_fon), list(sol_no3), list(sol_orgn), list(sol_aorgn), list(sol_orgp),
            list(sol_solp), list(sol_actp), list(
                sol_stap), list(sol_hum), sol_cov, sumno3,
