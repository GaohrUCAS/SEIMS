...
def FlowDirD8(np, workingDir, filledDem, flowDir, slope, mpiexeDir = None, exeDir = None):
    ...
    if exeDir is not None:
        ...
        exe = "d8flowdir"
    strCmd = strCmd + " %d %s -fel %s -p %s  -sd8 %s" % (np, exe, filledDem, flowDir, slope)
    print strCmd
    process = subprocess.Popen(strCmd, shell = True, stdout = subprocess.PIPE)
    return process.stdout.readlines()
...
def FlowDirDinf(np, workingDir, filledDem, flowAngle, slope, mpiexeDir = None, exeDir = None):
    ...
    if exeDir is not None:
        ...
        exe = "dinfflowdir"
    strCmd = strCmd + " %d %s -fel %s -ang %s -slp %s" % (np, exe, filledDem, flowAngle, slope)
    print strCmd
    process = subprocess.Popen(strCmd, shell = True, stdout = subprocess.PIPE)
    return process.stdout.readlines()
...
def FlowAccD8(np, workingDir, flowDir, acc, outlet = None, streamSkeleton = None, mpiexeDir = None, exeDir = None):
    ...
    if outlet is not None:
        if streamSkeleton is not None:
            strCmd = strCmd + " %d %s -p %s -o %s -wg %s -ad8 %s -nc" % (np, exe, flowDir, outlet, streamSkeleton, acc)
        else:
            strCmd = strCmd + " %d %s -p %s -o %s -ad8 %s -nc" % (np, exe, flowDir, outlet, acc)
    else:
        if streamSkeleton is not None:
            strCmd = strCmd + " %d %s -p %s -wg %s -ad8 %s -nc" % (np, exe, flowDir, streamSkeleton, acc)
        else:
            strCmd = strCmd + " %d %s -p %s -ad8 %s -nc" % (np, exe, flowDir, acc)
...
def StreamRaster(np, workingDir, acc, streamRaster, threshold = 1000, mpiexeDir = None, exeDir = None):
    ...
    if exeDir is not None:
        ...
        exe = "threshold"
    strCmd = strCmd + " %d %s -ssa %s -thresh %s  -src %s" % (np, exe, acc, str(threshold), streamRaster)
    print strCmd
    process = subprocess.Popen(strCmd, shell = True, stdout = subprocess.PIPE)
    return process.stdout.readlines()
...
def MoveOutlet(np, workingDir, flowDir, streamRaster, outlet, modifiedOutlet, mpiexeDir = None, exeDir = None):
    ...
    strCmd = strCmd + " %d %s -p %s -src %s -o %s -om %s" % (np, exe, flowDir, streamRaster, outlet, modifiedOutlet)
    print strCmd
    process = subprocess.Popen(strCmd, shell = True, stdout = subprocess.PIPE)
    return process.stdout.readlines()
...
def StreamSkeleton(np, workingDir, filledDem, streamSkeleton, mpiexeDir = None, exeDir = None):
    ...
    if exeDir is not None:
        ...
        exe = "peukerdouglas"
    strCmd = strCmd + " %d %s -fel %s -ss %s" % (np, exe, filledDem, streamSkeleton)
    print strCmd
    process = subprocess.Popen(strCmd, shell = True, stdout = subprocess.PIPE)
    return process.stdout.readlines()
