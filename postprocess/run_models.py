...
for par in parDict.keys():
    ...
    for wsID in watershedList:
        filenamePre = r'%s\model_dianbu%s_30m_longterm\OUTPUT%s\1_CH_%s.txt' % (baseFolder, wsID, 3, par)
        filenameAfter = r'%s\model_dianbu%s_30m_longterm\OUTPUT%s\1_CH_%s.txt' % (baseFolder, wsID, 2, par)
        varTotalPre += SumByFile(filenamePre, year)
        varTotalAfter += SumByFile(filenameAfter, year)
    liquid = (varTotalPre - varTotalAfter) / 1000.0
